...
private:
    cv::Point palm_center_affine_;//添加内容1 -add by sunzk
    std::vector<cv::Point> point_affine(const std::vector<cv::Point>& points, const cv::Mat& trans_mat);//添加内容1 -add by sunzk
    ...
    bool find_fingers(const cv::Mat& src, const cv::Rect& roi, std::list<cv::Point>& pts);
...
public:
    ...
    bool detect(const cv::Mat& src, const cv::Rect& roi, std::vector<cv::Point>& pts);
private:
